...
func request_item_interact(item_name: String, id = -1):
	MultiplayerConfirm.confirm_item_interact.rpc_id(1, item_name, id)
...
func request_item_spawn(item: Constants.ITEMS, position: Transform3D, id = -1):
	MultiplayerConfirm.confirm_item_spawn.rpc_id(1, item, position, id)
